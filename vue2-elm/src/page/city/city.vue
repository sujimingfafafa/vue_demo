...
export default {
  ...
  mounted(){
      let cityid=this.cityid =  this.$route.params.cityid;//获取点击页面上的Id
    ...
    // 发送搜索信息
        postpois(){
            ...
            if (this.inputVaule) {
                searchplace(this.cityid, this.inputVaule).then(res => {
                    this.historytitle = false;// 默认显示搜索历史头部，隐藏
                    this.placelist = res;
        ...
        nextpage(index, geohash){
          console.log(index,geohash)
            ...
            this.$router.push({path:'/msite', query:{geohash}})//点击跳转到详情页，query传参
        },
          clearAll(){
